...
import me.brynblack.primitiveprogression.init.ItemsInit;
    ...
    UseBlockCallback.EVENT.register(
        (player, world, hand, block) -> {
          ...
          if (isKnife && isRock && player.isSneaking()) {
            if (!world.isClient) {
              if (r1 <= PrimitiveProgression.CONFIG.flintConsumeProb) {
                world.breakBlock(pos, false);
                if (r2 <= PrimitiveProgression.CONFIG.flintSuccessProb && !player.isCreative()) {
